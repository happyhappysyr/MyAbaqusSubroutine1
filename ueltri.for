...
C     Assembling B Matrix
      ...
      BMATRIX(2,1) = 0
      BMATRIX(3,1) = BETA_J/(2*AREA)
      BMATRIX(4,1) = 0
      BMATRIX(5,1) = BETA_M/(2*AREA)
      BMATRIX(6,1) = 0
      BMATRIX(1,2) = 0
      BMATRIX(2,2) = GAMMA_I/(2*AREA)
      BMATRIX(3,2) = 0
      BMATRIX(4,2) = GAMMA_J/(2*AREA)
      BMATRIX(5,2) = 0
      BMATRIX(6,2) = GAMMA_M/(2*AREA)
      BMATRIX(1,3) = GAMMA_I/(2*AREA)
      BMATRIX(2,3) = BETA_I/(2*AREA)
      BMATRIX(3,3) = GAMMA_J/(2*AREA)
      BMATRIX(4,3) = BETA_J/(2*AREA)
      BMATRIX(5,3) = GAMMA_M/(2*AREA)
      BMATRIX(6,3) = BETA_M/(2*AREA)
...
C      MATRIX MULTIPLICATION FOR STIFFNESS MATRIX
      BTRANS = TRANSPOSE(BMATRIX)*(THICKNESS*AREA)
      KMATRIX = MATMUL(MATMUL(BTRANS,DMATRIX),BMATRIX)
